# iaq-modulation Specification

## Purpose

为 IAQ 语义通信的比特流提供数字调制层：BPSK 调制/硬判决解调，以及按 Eb/N0 的 AWGN 加噪。供 `awgn_channel` 复用，使信道误码强度由物理 SNR（Eb/N0）而非抽象翻转概率决定，未编码误码率符合 \(Q(\sqrt{2 \cdot \mathrm{Eb/N0}})\) 理论。
